...
client = Bot(description="gamers lounge Bot is best", command_prefix="d!", pm_help = True)
...
async def on_ready():
    ...
    print('--------')
    print('--------')
    print('Started GAMERS LOUNGE BOT')
    print('Created by 4D4M#7801')
...
async def on_member_join(member):
    ...
    channel = discord.utils.get(client.get_all_channels(), server__name='GAMERS LOUNGE Official Server', name='gamers lounge-servers-join-leave-log')
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
...
async def iamdark(ctx):
    ...
    print('Added GAMER role in ' + (ctx.message.author.name))
...
async def ownerinfo(ctx):
    embed = discord.Embed(title="Information about owner", description="Bot Name- GAMERS LOUNGE", color=0x00ff00)
    
    embed.set_author(name=" Bot Owner Name- 4D4M#7801")
    await client.say(embed=embed)
...
async def help(ctx):
    ...
    embed.add_field(name = '``Our Help Server Link`` ',value ='https://discord.gg/cFwqZYf',inline = False)
...
async def generalhelp(ctx):
    ...
    embed.add_field(name = 'd!donate ',value ='Use it to donate us and get a special post on Official Gamer lounge server.',inline = False)
...
client.run('NTU1NzIxMjk2MzcwMDczNjAx.D3FoPQ.M7EVE8IHxpI05cEmsmDdvQJaY8I')

...
async def on_member_join(member):
    ...
    channel = discord.utils.get(client.get_all_channels(), server__name='4D4M Official Server', name='4D4M-servers-join-leave-log')
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
...
async def ownerinfo(ctx):
    embed = discord.Embed(title="Information about owner", description="Bot Name- 4D4M BOT", color=0x00ff00)
...
async def generalhelp(ctx):
    ...
    embed.add_field(name = 'd!donate ',value ='Use it to donate us and get a special post on Official 4D4M BOT server.',inline = False)
